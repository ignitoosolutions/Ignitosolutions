...
This Flask app serves both the public facing pages and a secure
administrative portal.  Service information, contact submissions
and orders are persisted using SQLite via SQLAlchemy.  An initial
administrator account and a set of services are seeded into the
    ...
    if Service.query.count() == 0:
        services_seed = [
            ...
            },
            {
                ...
                'image_file': 'lead_generation.png'  # reuse funnel image for marketing bundle representation
            },
            {
...
@app.route('/')
def index():
    """Render the home page with a list of services."""
...
def add_service():
    ...
    if request.method == 'POST':
        ...
        service = Service(name=name, description=description, price_display=price_display, image_file=image_file)
